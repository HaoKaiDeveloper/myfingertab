<template>
  <section id="sheetMusicPage">
    <div class="header">
       ...
       <div class="select">
        <select>
        <option value="">難易度:由簡到難</option>
        <option value="">難易度:由難到簡</option>
        <option value="">熱門程度</option>
        <option value="">最新上架</option>
       </select>
       </div>
        <label for="search">樂譜搜尋
...
export default {
  ...
  setup(){
    ...
    Array.from(
      { length: Math.ceil(totalPages.value / 5) },
      (_, index) => {
        const start = index * 5;
        const end = (index + 1) * 5;
    ...
    function toggleMusicDetail(id){
      ...
      musicDetail.value=allMusic.value.find(m=>m.id===id);
      showMusicDetail.value=true;

    }
    

    async function getMusic(num,sortBy,search,page){
      const data=await store.dispatch('getAllMusic',{num:9,sortBy:'degreeEaseTOHard',page:1});
      allMusic.value=data
    }
